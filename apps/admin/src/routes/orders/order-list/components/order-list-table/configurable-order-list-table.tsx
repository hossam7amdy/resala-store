...
export const ConfigurableOrderListTable = () => {
  ...
  const { activeView, createView } = useViewConfigurations('orders')
  ...
  useEffect(() => {
    ...
    setSearchParams((prev) => {
      const keysToDelete = Array.from(prev.keys()).filter(
        (key) =>
          key.startsWith(QUERY_PREFIX + '_') ||
          key === QUERY_PREFIX + '_q' ||
          key === QUERY_PREFIX + '_order'
      )
  ...
  const hasConfigurationChanged = useMemo(() => {
    ...
    const currentSorting = queryParams.order
      ? parseSortingState(queryParams.order)
      : null
  ...
  const filterBarContent = debouncedHasConfigChanged ? (
    <>
      ...
      <SaveViewDropdown
        isDefaultView={
          currentActiveView?.is_system_default || !currentActiveView
        }
